...
public class HorarioDeAtencion {
	...
	public void agregarRangoDeHorario(DayOfWeek dia, RangoDeHorario rangoDeHorario) {
		...
		horarioSemanal.putIfAbsent(dia, new ArrayList<RangoDeHorario>());// Si todavia no estaba el dia entre los horarios lo agrego
		horarioSemanal.get(dia).add(rangoDeHorario);
	}

	public boolean estaDentroDeRangosDeHorario(LocalDateTime diaYHora) {
		return horarioSemanal.containsKey(diaYHora.getDayOfWeek()) &&
				horarioSemanal.get(diaYHora.getDayOfWeek())
				.stream()
